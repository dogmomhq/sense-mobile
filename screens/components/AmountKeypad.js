...
// hideDisplay: the caller already draws the big figure and the chips (the deposit sheet does, so the
// keypad is just the pad). compact: tighter rows, for a sheet where the pay button must stay visible.
export default function AmountKeypad({ value, onChange, chips = [], maxCents = null, allowCents = true, disabled = false, hint = null, tone = COLORS.cream, hideDisplay = false, compact = false }) {
  const s = useScale();
  const press = (k) => {
    if (disabled) return;
    let v = String(value || '');
  ...
  return (
    <View>
      ...
      {hideDisplay ? null : hint ? <Text style={{ fontFamily: FONTS.interSemi, fontSize: 24 * s, color: COLORS.creamDim, textAlign: 'center', marginBottom: 20 * s, letterSpacing: 0.04 * 24 * s }}>{hint}</Text> : <View style={{ height: 20 * s }} />}
      {hideDisplay || !chips.length ? null : (
      ...
      <View style={{ marginHorizontal: 40 * s }}>
        {keys.map((row, i) => (
          <View key={i} style={{ flexDirection: 'row' }}>
            {row.map((k, j) => (
              <Pressable key={j} onPress={() => k && press(k)} disabled={!k || disabled} style={({ pressed }) => ({ flex: 1, alignItems: 'center', justifyContent: 'center', paddingVertical: (compact ? 22 : 26) * s, opacity: pressed ? 0.5 : 1 })}>
                <Text style={{ fontFamily: FONTS.interBold, fontSize: (k === 'back' ? 60 : compact ? 66 : 76) * s, color: COLORS.cream, includeFontPadding: false }}>{k === 'back' ? '⌫' : k}</Text>
              </Pressable>))}
          </View>))}
      </View>
    </View>
